...
Status DBImpl::SyncWAL() {
...

    {
        ...
        // FIXME 在进行log_sync_cv_.SignalAll的过程中,log_预留了一个实例，并在后续将标志设置为true，这里会不会产生阻塞？
...
Status DBImpl::GetImpl(const ReadOptions& read_options,
    ...
    // 内部key的查找需要构造一个internal key，记录了type，seq信息，由LookupKey来构造
...
Status DBImpl::CreateColumnFamilyImpl(const ColumnFamilyOptions& cf_options,
    ...
    *handle = nullptr;
    ...
    SuperVersionContext sv_context(/* create_superversion */ true);
    {
        ...
        // 这里取出创建好的ColumnFamilyData object
        if (s.ok()) {
            auto* cfd =
        ...
        if (s.ok()) {
            ...
            //更新sv_context state，并考虑是否需要flush/compaction
